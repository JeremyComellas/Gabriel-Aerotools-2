...
# This is a copy of Stitching_Matching_TRACKER.py, to accomodate all the improvements done on that code.
# (I NEED TO REVERT THE MASK)
...
def stitching_suelo_MULT(im_num0, im_numf, num_matches, pix_overlap, correction=0,  logger=True, plot=False, plotMASK=False, plot_altering=False, plot_matches=False, version="", folder_tag="",folder_save="Data_processed/STITCHES_FLOOR_MULT_ORDERED/", folder_path_input="Data_processed/Split_ini/__0/",  compare=False, record=False, contar= False, INFO_MATCH=False):
    ...
    for i in range(ini, fini+1):
        print_n_save("==================================================================================================================")
        print_n_save(f"Starting to save image number {i} out of {fini} \nWhich is {(i-ini)/(fini-ini+0.00001)*100:.2f}%, after {(time.time()-start_time):.2f} seconds from begining")
        print_n_save("==================================================================================================================")
        previous_time = time.time()
        Current_image_time = time.time()
        TIMESS.append(Current_image_time-start_time)
        print_n_save(f"TIMESS:   {TIMESS}")
        ...
        def create_mask_TRACKER(image, boxes, porcen=0):
            ...
            for box in boxes:
                ...
                mask[int(y1+pixels_cut):int(y2-pixels_cut), int(x1):int(x2)] = 0
        ...
        if plotMASK:
            ...
            def plot_mask(image, mask):
                ...
                masked_image[mask == 0] = [0, 0, 0]  # Red color for masked areas
                ...
                # plt.axis('off')
                plt.savefig(directory_path+"Masks")
                plt.show()
            print_n_save("Plots of created masks")
            plot_mask(image1, mask_1)
            plot_mask(image2, mask_2)
        ...
        image1_feat = cv2.cvtColor(image1_feat, cv2.COLOR_BGR2GRAY)
        image2_feat = cv2.cvtColor(image2_feat, cv2.COLOR_BGR2GRAY)

        if plot_altering:
            mostrar2(image1_feat, image2_feat, "Grayscale images")
        ...
        image1_feat = clahe.apply(image1_feat)
        image2_feat = clahe.apply(image2_feat)
        ...
        if plot_matches:
            ...
            mostrar(img_matches)
            if record:
                cv2.imwrite(directory_path + f"Matches_Images_{ini}_{i}_{version}.jpg", img_matches)    
        ...
        def rejuntar_orig(image1, image2, start_y=0):
            height1, width1, _ = image1.shape
            height2, width2, _ = image2.shape
        ...
        def rejuntar_inv(image1, image2, start_y=0):
            height1, width1, _ = image1.shape
            height2, width2, _ = image2.shape
        ...
        if mode == "orig":
            joined_image = rejuntar_orig(im_BEG, joined_image, start_y)

        elif mode == "inv":
            joined_image = rejuntar_inv(joined_image, im_END, start_y)   
        ...
        if compare:
            ...
            def mostrar_comp(image1, image2, im_comb, image_numb1, image_numb2, start_x, mode, record=False, sav_title=directory_path):
                    ...
                    plt.tight_layout()
                    if record:
                        plt.savefig(sav_title+mode+f"__{ini}_{i}")
                    else:
                        plt.show()
                    plt.close()
        ...
        previous_time = time.time()        
        if record:
            cv2.imwrite(directory_path + f"Images_{ini}_{i}_{version}.jpg", joined_image)    
    ...
    print_n_save(f"You can find the outputs in {directory_path}")
    print_n_save("/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/")
    print_n_save("\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/")
    print_n_save("/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/")
    print_n_save("\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/")
...
PEGADITA1 = stitching_suelo_MULT(721, 761, 100, pix_overlap=2, correction=0, version="", folder_tag="_COMPARISON_TIMES", plot=False, plotMASK=False, plot_altering=False, plot_matches=False, compare=False, logger=True, record=True, folder_save="Data_processed/STITCHES_FLOOR_MULT_ORDERED/", folder_path_input="Data_processed/Split_ini/__0/",  INFO_MATCH=False)




# consider using SUPERGLUE or whatever is called. 
# If you are looking for higher accuracy or robustness, consider alternative detectors like ORB (if you need a faster solution) or more recent deep learning-based methods like SuperGlue, which might offer improved performance in challenging scenarios.


# Pegar las imagenes originales, no las grayscale. Osea hacer una segunda copia de RGB PARA LA DETECCIÓN CON SIFT.